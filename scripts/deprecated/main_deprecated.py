...
from scrollables import ListItem, ScrollableList
...
class SearchApp(QWidget):
    def __init__(self):
        ...
        dropdown_menu.addItems(["Title", "Author", "Keywords", "Conference"])
        dropdown_menu.setStyleSheet("color: white; background-color: #303030;")

        # Variable to store the selected item
        self.order_by = None

        # Connect the signal to the function to update the variable
        dropdown_menu.currentIndexChanged.connect(self.update_order_by)
        ...
        search_layout.addWidget(dropdown_menu)
...
#####
        ...
        'Related Papers': [
            {'Title': 'Related Paper 1', 'Author': 'Jane Doe', 'ref': 100, 'keywords': 'NLP, ML'},
            {'Title': 'Related Paper 2', 'Author': 'Bob Smith', 'ref': 10, 'keywords': 'NLP, ML'},
        ]
        }
...
class ScrapViewer(QFrame) :
    ...
    def initUI(self) :
        title_label = QtWidgets.QLabel("CLIPS")
        title_label.setStyleSheet("color: white; font-size: 18px; border-bottom: 1px solid white; font-weight: bold;")

        paper_list = ScrollableList()

        layout = QVBoxLayout()
        layout.addWidget(title_label)
        layout.addWidget(paper_list)
        self.setLayout(layout)

    def update(self, item_list) :
        self.paper_list.update(item_list)
